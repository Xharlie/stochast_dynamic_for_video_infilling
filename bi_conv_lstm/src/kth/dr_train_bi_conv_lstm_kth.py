...
def conditional_addframes(seq, section, mode, B, T, K):
    length = B*(K+T)+K
    full_seq = np.zeros((seq.shape[0], seq.shape[1], length, seq.shape[3]))
    if mode.split("_")[1] == "progressive":
...
def main(lr, batch_size, alpha, beta, image_size_h, image_size_w, K, T, B, convlstm_layer_num, num_iter, gpu, cpu,
         ...
         reference_mode, debug, dis_length, mode, Unet, no_d, fade_in, use_gt, res_mode, concatconv):
    ...
    if len(train_tf_record_files) == 0:
        ...
        train_vids = load_save_data.load_records(glob.glob(tf_record_train_dir + '*tfrecords*'), length=B*(K+T)+K)
    ...
    with tf.device(device_string):
        model = bi_convlstm_net(dec=dec, image_size=[image_size_h, image_size_w], c_dim=color_channel_num, dis_length=dis_length,
                                ...
                                debug=debug, reference_mode=reference_mode, mode = mode, Unet= Unet, use_gt = use_gt, res_mode=res_mode,
                                pixel_loss=pixel_loss, concatconv=concatconv)
                    ...
                    if np.mod(counter, 2500) == 0:
                        ...
                        for i in range(3):
                            sample_train = depth_to_width(conditional_addframes(
                                samples_train[i], (K+T) // (2**num_layers), mode, B, T, K).swapaxes(0, 2).swapaxes(1, 2))
                            ...
                            sample_test = depth_to_width(conditional_addframes(
                                samples_test[i], (K+T) // (2**num_layers), mode, B, T, K).swapaxes(0, 2).swapaxes(1, 2))
...
if __name__ == "__main__":
    ...
    # bi vs single: each layer has bidirection or single lstm
    # progressive vs one: many layers or one layer
    # serial or parallel: in each layer, concat directional or single forward + backward
    # bi_progressive_serial, bi_progressive_parallel or bi_one_parallel, bi_one_serial,
    # single_progressive_serial, single_progressive_parallel,
