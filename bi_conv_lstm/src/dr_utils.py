...
def load_triplet_data_from_list(train_vids, batchidx, alter_batchidx, image_size_h, image_size_w, nShare, maxStep, flipable=True,
    ...
    for i in range(len(batchidx)):
        ...
        # print selected.shape,diff_selected.shape
        while True:
            low = 0
        ...
        if i < len(batchidx):
            ...
            for j in range(nShare):
                # print seq[i, :, :, :].shape
                seq[i, :, :, j*channel:j*channel + channel] = transform(selected[:, :, stidx + j, :])
                seq_same[i, :, :, j*channel:j*channel + channel] = transform(selected[:, :, offset_start + j, :])
                seq_diff[i, :, :, j*channel:j*channel + channel] = transform(diff_selected[:, :, stidx_diff + j, :])
...
# decode from avi
def load_avi_data(f_name, data_path, image_size_h, image_size_w, channel=3):
  flip = np.random.binomial(1,.5,1)[0]
  vid_path = data_path + f_name
  vid = imageio.get_reader(vid_path,"ffmpeg")
  seq = np.zeros((image_size_h, image_size_w, vid.get_length(), channel), dtype="float32")
  for t in xrange(vid.get_length()):
    img = cv2.resize(vid.get_data(t),
