...
class bi_convlstm_net(object):
    def __init__(self, image_size, batch_size=32, c_dim=3,
                 ...
                 is_train=True, debug = False, reference_mode = "two", pixel_loss="l2",concatconv=False,
                 convlstm_kernel = [3, 3], dec = 'normal', mode="bi_one_serial", Unet = True, use_gt=True,res_mode="mix"):
        self.dec = dec
        self.res_mode = res_mode
        self.concatconv = concatconv
        self.use_gt = use_gt
    ...
    def build_model(self):
        ...
        if self.mode == "bi_one_serial" or self.mode == "bi_one_parallel":
    ...
    def forward(self, for_seq, is_gen):
        ...
        frames = []
        original_seq = None
        for_seq, res = self.pooling_feature_enc(for_seq, reuse=reuse, is_progressive=False)
        if self.use_gt: original_seq = tf.identity(for_seq)
        for_seq = self.convRnn_seq_op(self.convlstm_layer_num - 1, original_seq, for_seq, reuse=reuse)
        if self.debug and self.use_gt:
            print "original_seq.get_shape().as_list(), for_seq.get_shape().as_list()", original_seq.get_shape().as_list(), for_seq.get_shape().as_list()
    ...
    def conditional_use_gt(self, seq, index, original_seq):
        if self.use_gt:
            ...
            if self.mode == "bi_one_serial" or self.mode == "bi_one_parallel":
                for t in range(0, self.length, self.K+self.T):
                    if t != 0:
                        seq_sec = seq[:, t - self.K - self.T + 1: t, :, :, :]
                        grounded_seq = tf.concat([grounded_seq, seq_sec, tf.expand_dims(original_seq[:,t,:,:,:],1)], axis=1)
                    if self.debug:
                        print grounded_seq.get_shape().as_list()
                return grounded_seq
            else:
                section = 2 ** (index + 1)
                for t in range(1, self.B + self.K):
                    seq_sec = seq[:, (t - 1) * section + 1: t * section, :, :, :]
                    # if (section==2): seq_sec = tf.expand_dims(seq_sec,1)
                    grounded_seq = tf.concat([grounded_seq, seq_sec, tf.expand_dims(original_seq[:, t, :, :, :], 1)],
                                             axis=1)
                return grounded_seq
        else:
            return seq

    def convRnn_seq_op(self, unit_index, original_seq, seq, reuse=tf.AUTO_REUSE):
        ...
        if self.mode.startswith('single'):
            ...
            if self.mode.split("_")[2] == 'serial':
                with tf.variable_scope('for_convRnn_' + str(unit_index), reuse=reuse):
                    for_cell = ConvGRUCell(shape, self.gf_dim * 2, self.convlstm_kernel)
                    outputs, state = tf.nn.dynamic_rnn(for_cell, seq, dtype=seq.dtype)
                if self.use_gt:
                    seq = self.conditional_use_gt(outputs, unit_index, original_seq)
                seq = tf.reverse(seq, axis=[1])
                with tf.variable_scope('back_convRnn_' + str(unit_index), reuse=reuse):
                    back_cell = ConvGRUCell(shape, self.gf_dim * 2, self.convlstm_kernel)
                    outputs, state = tf.nn.dynamic_rnn(back_cell, seq, dtype=seq.dtype)
    ...
    def assemble_hidden(self, seq, i, reuse=tf.AUTO_REUSE, train=True):
        ...
        blank = tf.zeros([seq_shape[0], 1, self.image_size[0], self.image_size[1], self.c_dim])
        for j in range(1, (2**(i+1)) * self.B + self.K):
            if j % 2 == 0:
                ...
                if self.concatconv:
                    ...
                    concat_seq = tf.concat([concat_seq, blank], axis=1)
        return concat_seq
    ...
    def forward_progressively(self, for_seq, is_gen):
        ...
        for i in range(self.convlstm_layer_num):
            ...
            for_seq = self.convRnn_seq_op(i, original_seq,
