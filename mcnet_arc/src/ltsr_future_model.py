...
class ltsr_future_net(object):
    ...
    def build_model(self, recon_lost):
        ...
        cell = BasicConvLSTMCell([self.image_size[0] / 8, self.image_size[1] / 8],
                                 [3, 3], 256)
        pred, h_dyn_lst, enc_h_lst = self.forward(self.target, self.xt, cell)
    ...
    def forward(self, seq_in, xt, cell):
        # Initial state
        state = tf.zeros([self.batch_size, self.image_size[0] / 8,
                          self.image_size[1] / 8, 512])
        reuse = False
        # store the h_dyn for every frame:
        h_dyn_lst=[]
        # store the enc_h for given frames:
        enc_h_lst=[]

        # Encoder
        for t in xrange(self.K):
            enc_h, res_m = self.motion_enc(seq_in[:, :, :, t, :], reuse=reuse)
            enc_h_lst.append(enc_h)
            h_dyn, state = cell(enc_h, state, scope='lstm', reuse=reuse)
            h_dyn_lst.append(h_dyn)
            reuse = True
        ...
        for t in xrange(self.T):
            if t == 0:
                # subtract last enc_h as last given frame's dynamic representation
                h_tp1 = self.comb_layers(tf.subtract(h_dyn-enc_h_lst[-1]), h_cont, reuse=False)
                x_hat = self.dec_cnn(h_tp1, res_c, reuse=False)
            else:
                h_dyn, state = cell(h_dyn, state, scope='lstm', reuse=True)
                h_dyn_lst.append(h_dyn)
                h_tp1 = self.comb_layers(tf.subtract(h_dyn-enc_h_lst[-1]), h_cont, reuse=True)
                # res_connect = self.residual(res_m, res_c, reuse=True)
                # just send res_c as residual
                x_hat = self.dec_cnn(h_tp1, res_c, reuse=True)
        ...
        return pred, h_dyn_lst, enc_h_lst
